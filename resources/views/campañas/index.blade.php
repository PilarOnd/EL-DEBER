   ...
   <div class="container mt-4">
        <div class="row text-center align-items-center" style="background-image: url('/images/background.jpg'); background-size: cover; padding: 20px; border-radius: 10px;">
            ...
            <div class="col-md-3">
                ...
                <p class="mb-0">{{ $plataforma['tipo'] }} <br> act. {{ date('d/m/Y', strtotime($plataforma['actualización'])) }}</p>
            </div>

            <div class="col-md-3">
                <h5 class="fw-bold text-success">PRESUPUESTO</h5>
                <h4 class="fw-bold">{{ number_format($presupuesto['monto'], 0, '', '.') }} ${{ strtoupper($presupuesto['moneda']) }}</h4>
                ...
                <p class="mb-0">{{ number_format($presupuesto['monto'], 0, '', '.') }} ${{ strtoupper($presupuesto['moneda']) }}</p>
            </div>
        </div>
    </div>
